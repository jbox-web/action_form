...
RSpec.describe('SingleModelForm') do
  ...
  it('declares form attributes') do
    attributes = %i[name name= age age= gender gender=]
  ...
  it('validates itself') do
    ...
    %i[name age gender].each do |attribute|
      expect(@form.errors.messages[attribute]).to include("can't be blank")
    end
    @form.name = 'Peters'
    @form.age = 23
    @form.gender = 0
  ...
  it('does not save the model with invalid data') do
    ...
    @form.submit(params)
    expect { @form.save }.to_not(change(User, :count))
    expect(@form.valid?).to(be(false))
    expect(@form.errors.messages[:name]).to include('has already been taken')
    expect(@form.errors.messages[:gender]).to include("can't be blank")
  end
